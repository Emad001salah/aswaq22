...
function getCategoryKey(cat: any): string {
  ...
  // Check specific compound names first!
  if (id === 'car_rental' || nameAr.includes('تأجير سيارات') || nameEn.toLowerCase().includes('car rental')) return 'car_rental';
  if (id === 'cars' || (nameAr.includes('سيارات') && !nameAr.includes('تأجير')) || nameAr.includes('مركبات') || nameEn.toLowerCase().includes('car')) return 'cars';
  if (id === 'rentals' || nameAr.includes('سكن') || nameAr.includes('إيجار') || nameEn.toLowerCase().includes('rent')) return 'rentals';
  if (id === 'realestate' || nameAr.includes('عقارات') || nameAr.includes('أراضي') || nameEn.toLowerCase().includes('real')) return 'realestate';
  if (id === 'hotels' || nameAr.includes('فنادق') || nameEn.toLowerCase().includes('hotel')) return 'hotels';
  if (id === 'resorts' || nameAr.includes('منتجعات') || nameEn.toLowerCase().includes('resort')) return 'resorts';
  if (id === 'electronics' || nameAr.includes('إلكترونيات') || nameEn.toLowerCase().includes('electronic')) return 'electronics';
  if (id === 'phones' || nameAr.includes('هواتف') || nameAr.includes('جوالات') || nameEn.toLowerCase().includes('phone')) return 'phones';
  if (id === 'laptops' || nameAr.includes('كمبيوتر') || nameAr.includes('لابتوب') || nameEn.toLowerCase().includes('laptop')) return 'laptops';
  if (id === 'furniture' || nameAr.includes('أثاث') || nameEn.toLowerCase().includes('furniture')) return 'furniture';
  if (id === 'clothing' || nameAr.includes('ملابس') || nameAr.includes('موضة') || nameEn.toLowerCase().includes('clothing')) return 'clothing';
  if (id === 'jobs' || nameAr.includes('وظائف') || nameAr.includes('فرص') || nameEn.toLowerCase().includes('job')) return 'jobs';
  if (id === 'services' || nameAr.includes('خدمات') || nameAr.includes('صيانة') || nameEn.toLowerCase().includes('service')) return 'services';
  if (id === 'livestock' || nameAr.includes('مواشي') || nameAr.includes('حيوانات') || nameEn.toLowerCase().includes('livestock')) return 'livestock';
  if (id === 'bicycles' || nameAr.includes('دراجات') || nameEn.toLowerCase().includes('bike')) return 'bicycles';
  if (id === 'trucks' || nameAr.includes('شاحنات') || nameAr.includes('معدات') || nameEn.toLowerCase().includes('truck')) return 'trucks';
  if (id === 'educational' || nameAr.includes('كتب') || nameAr.includes('تعليم') || nameEn.toLowerCase().includes('education')) return 'educational';
  if (id === 'food' || nameAr.includes('أغذية') || nameAr.includes('مأكولات') || nameEn.toLowerCase().includes('food')) return 'food';
  if (id === 'medical' || nameAr.includes('طبية') || nameAr.includes('صحة') || nameEn.toLowerCase().includes('medical')) return 'medical';
  if (id === 'perfumes' || nameAr.includes('عطور') || nameAr.includes('تجميل') || nameEn.toLowerCase().includes('perfume')) return 'perfumes';
  if (id === 'construction' || nameAr.includes('بناء') || nameAr.includes('مقاولات') || nameEn.toLowerCase().includes('construction')) return 'construction';
  if (id === 'custom_work' || nameAr.includes('يدوية') || nameAr.includes('حرف') || nameEn.toLowerCase().includes('craft')) return 'custom_work';
...
export default function Hero({
  ...
  return (
    <div className={`border-b transition-colors duration-305 ${isRtl ? 'dir-rtl text-right' : 'dir-ltr text-left'} ${isDark ? 'bg-slate-950 border-slate-900' : 'bg-white border-slate-200'}`}>
      ...
      <div className={`w-full relative h-[400px] md:h-[450px] overflow-hidden flex flex-col items-center justify-center transition-colors ${isDark ? 'bg-gradient-to-br from-emerald-950 via-teal-950 to-slate-950' : 'bg-gradient-to-br from-emerald-100/90 via-teal-50/70 to-emerald-100/40'}`}>
        <div
          className={`absolute inset-0 bg-cover bg-center mix-blend-luminosity ${isDark ? 'opacity-30' : 'opacity-15'}`}
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1542459955-44cb294975d0?auto=format&fit=crop&w=1920&q=80')",
          }}
        />
        <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-t from-slate-950 via-slate-950/70 to-transparent' : 'bg-gradient-to-t from-white/90 via-emerald-50/30 to-transparent'}`} />
        <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-r from-emerald-900/60 to-transparent' : 'bg-gradient-to-r from-emerald-400/20 via-teal-400/10 to-transparent'}`} />

        <div className="relative z-10 px-4 text-center max-w-4xl mx-auto flex flex-col items-center -mt-16 sm:-mt-20">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] font-black mb-6 backdrop-blur-md uppercase tracking-[0.2em] shadow-lg ${isDark ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 shadow-emerald-500/5' : 'bg-emerald-100/80 border-emerald-300 text-emerald-800 shadow-emerald-500/10'}`}>
          ...
          <p className={`text-sm md:text-lg max-w-2xl font-bold leading-relaxed mb-8 opacity-90 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
            {t('hero.description')}
          </p>
